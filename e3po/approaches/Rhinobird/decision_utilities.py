...
def predict_motion_tile1(motion_history, motion_history_size, motion_prediction_size):
    ...
    # get_logger().debug(f'motion_history_size: {motion_history_size} motion_prediction_size: {motion_prediction_size}')
    # get_logger().debug(f'hw: {hw}')
    ...
    # get_logger().debug(f'predicted_motion: {predicted_motion}')
    predicted_record = []
    for i in range(motion_prediction_size):
        predicted_record.append(deepcopy(predicted_motion))

    # get_logger().debug(f'predicted_record: { predicted_record}')
...
def predict_motion_tile0(motion_history, motion_history_size, motion_prediction_size):
    ...
    hw_yaw = [d['motion_record']['yaw'] for d in motion_history]
    hw_pitch = [d['motion_record']['pitch'] for d in motion_history]

    # get_logger().debug(f'motion_history: {motion_history}')
    if len(hw_yaw) < (motion_history_size//10):
        return predict_motion_tile1(motion_history, motion_history_size, motion_prediction_size)
    
    # Create a Linear Regression model for yaw
    lr_yaw = LinearRegression()
    lr_pitch = LinearRegression()
    sequence = np.array([i * 0.01 for i in range(motion_history_size//10)]).reshape(-1,1)
    hw_yaw = np.array(list(hw_yaw)[-(motion_history_size//10):]).reshape(-1,1)
    hw_pitch = np.array(list(hw_pitch)[-(motion_history_size//10):]).reshape(-1,1)

    # Fit the Linear Regression model for latitude
    lr_yaw.fit(sequence, hw_yaw)
    lr_pitch.fit(sequence, hw_pitch)
...
def generate_dl_list(chunk_idx, tile_record, latest_result, dl_list):
    ...
    get_logger().debug(f'tile_record: {tile_record}')
    get_logger().debug(f'latest_result: {latest_result}')
    tile_result = []
    ...
    tile_result = list(set(tile_result))
